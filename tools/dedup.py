    ...
    if args.extract:
        ...
        for src in srcs:
            ...
            for message in original:
                deduped[(message['name'], message['message'])] = message
        ...
        for src in srcs:
            ...
            for i, message in enumerate(original):
                if (message['name'], message['message']) in deduped_dict:
                    original[i]['message'] = deduped_dict[(message['name'], message['original'])]['message']
                else:
                    if message['message']:
