...
from extract import TITLE_PREFIX, LABEL_PREFIX, SKIP_PREFIX, LINE_PATTERN
...
if __name__ == '__main__':
    ...
    for txt in original_scenario_dir.glob('*.txt'):
        ...
        messages = iter(load(translated_json_dir / (txt.name + '.json')))
        ...
        for line in lines:
            ...
            if line.startswith(TITLE_PREFIX):
                ...
                for _p, _n in NAME_DICT.items():
                    message.message = message.message.replace(_n, _p)
                for h, f in FW_NUMS.items():
                    message.message = message.message.replace(h, f)
                data.append(message.message.encode('cp936'))
