...
ACTOR_PATTERN = re.compile(r'\A(.*?)([\s*「『（][\s\S]*)\Z')
...
def text_reader(reader):
    for row in reader:
        yield row.get('Original text', '')
...
def parse_name(text):
    if match := ACTOR_PATTERN.search(text):
        ...
        if ( _text and
            (
                (_text[0] in BRACKETS and BRACKETS[_text[0]] == _text[-1])
                or (_text[0] not in BRACKETS and 0 < len(_name) < 9)
            )
        ):
            return _name, match.group(2)

    return '', text

def extract_file(csv_path: Path, message_list: MessageList) -> int:
    with open(csv_path, newline='') as f:
        treader = text_reader(csv.DictReader(f))
        for text in treader:
            if not text:
                continue
