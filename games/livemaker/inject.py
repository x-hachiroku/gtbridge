...
def inject_file(csv_path: Path, json_path: Path, out_path: Path) -> None:
    ...
    with open(csv_path, newline='') as f_in, \
        ...
        for row in reader:
            ...
            _, _original_text = parse_name(original_text)

            if not _original_text.strip():
                writer.writerow(row)
                continue

            msg = next(messages)
            assert msg.name + msg.original == original_text
            translated_text = NAMES.get(msg.name, msg.name) + msg.message
