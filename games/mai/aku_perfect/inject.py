    ...
    for s in SCT_DICT:
        ...
        for chunk in sct.chunks:
            ...
            for i, message in enumerate(messages):
                ...
                assert translated_message.original == message, (s, chunk.name, translated_message.original, message)
                if len(translated_message.message) > 120:
                    print(f'Warning: message too long in {chunk.name}: "{translated_message.message}"')
                translated_message.message = translated_message.message.replace(FN, '#fn').replace(SN, '#sn')
